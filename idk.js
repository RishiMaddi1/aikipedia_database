  ...
  "google/gemini-2.5-flash-preview": { input: 12.6, output: 50.4 },
    ...
    if (req.method === 'POST' && new URL(req.url).pathname === '/chat') {
      try {
        ...
        // Prepare system prompt (optional)
        const systemPrompt = AVAILABLE_MODELS[model]?.systemPrompt
          ? { role: "system", content: AVAILABLE_MODELS[model].systemPrompt }
          : null;
    
        const trimmedHistory = history.slice(-12);
        const messages = systemPrompt
          ? [systemPrompt, ...trimmedHistory, { role: "user", content: message }]
